...
import { readFileSync, existsSync, statSync, rmSync, readdirSync, mkdirSync, cpSync } from 'node:fs'
...
/** 批量识别搜索结果类型（官方 bundle / 聚合仓库 / 普通项目）。
...
async function enrichItems(items) {
  return Promise.all(items.map(async (item) => {
    ...
    try {
      ...
      const pkgText = await Promise.any([
        curlText(base, 4000),
    ...
    return { ...item, official, aggregate, aggregateInstallable }
  }))
}

  ...
  try {
    ...
    if (Array.isArray(data.registries) && data.registries.length > 0) {
      const registries = data.registries
        .filter((r) => r && typeof r.url === 'string' && /^https:\/\/\S+$/u.test(r.url))
        .map((r) => ({
...
/** 任意 https JSON 接口（Gitee/GitLab 等平台搜索用）。 */
  ...
  try {
    ...
    for (let index = 0; index < candidates.length && installedName === null; index += 1) {
      ...
      if (installedName === null) {
        // 通道 n+1：git 通道（GitHub 走加速代理+直连；Gitee/GitLab 走对应平台；各 60 秒封顶）
        if (!subpackageMode && !expanded) {
          const gitSpecs = job.source === 'gitee'
            ? [`git+https://gitee.com/${job.repo}.git`]
            : job.source === 'gitlab'
              ? [`git+https://gitlab.com/${job.repo}.git`]
              : [
...
async function handle(ctx, req, res) {
  ...
  if (method === 'GET' && pathname === `${ROUTE_PREFIX}/sources`) {
    sendJson(res, 200, { ok: true, sources: readSources() })
    return
  }
  ...
  if (pathname === `${ROUTE_PREFIX}/search`) {
    ...
    const source = body.source === 'gitee' || body.source === 'gitlab' ? body.source : 'github'
    const auth = readGithubAuth()
    let items = []
    if (source === 'gitee') {
      const data = await fetchJsonUrl(`https://gitee.com/api/v5/search/repositories?q=${encodeURIComponent(query)}&page=${page}&per_page=20&sort=stars_count&order=desc`)
      ...
      const data = await fetchJsonUrl(`https://gitlab.com/api/v4/projects?search=${encodeURIComponent(query)}&per_page=20&page=${page}&order_by=star_count&sort=desc`)
      items = (Array.isArray(data) ? data : []).map((item) => ({
        fullName: String(item.path_with_namespace ?? '').trim(),
        description: item.description ?? '',
  ...
  if (pathname === `${ROUTE_PREFIX}/sources`) {
    ...
    if (action === 'add') {
      ...
      if (!/^https:\/\/\S+$/u.test(url)) {
        sendError(res, 400, '软件源地址必须是 https:// 开头的合法 URL')
        return
      }
    ...
    if (action === 'remove') {
      ...
      const removed = sources.registries.find((r) => r.id === id)
      if (!removed) {
        sendError(res, 404, '没有这个软件源')
        return
      }
      sources.registries = sources.registries.filter((r) => r.id !== id)
      if (removed.primary && sources.registries.length > 0) sources.registries[0].primary = true
      await writeSources(sources)
      sendJson(res, 200, { ok: true, sources })
      return
    }
    ...
    sendError(res, 400, '未知操作（add / remove / set-primary / reset）')
    return
  }

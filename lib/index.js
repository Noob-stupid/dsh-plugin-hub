  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        // npm 安装函数：Start-Process + 输出重定向到文件（schtasks 任务 stdout 管道可能阻塞 npm——
        // 曾导致 npm 无声卡死）+ 10 分钟总时长硬超时（网络黑洞时 fetch 可能无限挂起）
        `function Install-Framework($reg) {`,
        `  $npmOut = $log + '.npm.out.txt'`,
        `  $proc = Start-Process -FilePath ${ps(nodePath)} -ArgumentList @(${ps(npmCliJs)}, 'install', '@deepseek-ai/dsh@${target}', '--no-audit', '--no-fund', '--registry', $reg, '--fetch-timeout=45000', '--fetch-retries=1', '--fetch-retry-mintimeout=5000') -RedirectStandardOutput $npmOut -RedirectStandardError ($npmOut + '.err') -PassThru -WindowStyle Hidden`,
        `  if (-not $proc.WaitForExit(600000)) { Stop-Process -Id $proc.Id -Force -ErrorAction SilentlyContinue; Log 'npm 超过 10 分钟未完成，已强制终止'; return 1 }`,
        `  try { return $proc.ExitCode } catch { Log ('读取 npm 退出码失败：' + $_.Exception.Message); return 1 }`,
        `}`,

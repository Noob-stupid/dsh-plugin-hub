  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `  try { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if (-not $c) { $psix = New-Object System.Diagnostics.ProcessStartInfo; $psix.FileName = '${nodePath}'; $psix.Arguments = '${binPath} web'; $psix.UseShellExecute = $false; $psix.CreateNoWindow = $true; [System.Diagnostics.Process]::Start($psix) | Out-Null } } catch {}`,
        ...
        // semver 范围内时 npm 会跳过安装，假成功）+ 10 分钟总时长硬超时。注意：**不能加
        ...
        `    $psi.Arguments = '${npmCli} install @deepseek-ai/dsh@${target} --force --no-audit --no-fund --no-save --registry ' + $reg + ' --fetch-timeout=45000 --fetch-retries=1 --fetch-retry-mintimeout=5000'`,
        ...
        `  if (-not $ok -and -not $started) { if (Test-Path ${ps(binPath)}) { $psir = New-Object System.Diagnostics.ProcessStartInfo; $psir.FileName = '${nodePath}'; $psir.Arguments = '${binPath} web'; $psir.UseShellExecute = $false; $psir.CreateNoWindow = $true; [System.Diagnostics.Process]::Start($psir) | Out-Null; $started = $true; Log '已发起拉起服务' } else { Log 'bin.js 缺失，跳过拉起' } }`,

  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        // npm 安装函数：Start-Process 直跑 cli.js（绕过 npm-cli.js 的 spawn 卡死）+ 10 分钟
        // 总时长硬超时（网络黑洞时 fetch 可能无限挂起）。注意：**不能加
        // -RedirectStandardOutput/-RedirectStandardError**——schtasks 任务环境下带重定向的
        // Start-Process 子进程会启动即异常退出（ExitCode 读不到、输出 0 字节，实测确认）；
        // 无重定向时 ExitCode 正常（实测 exit=0）。npm 自身 debug 日志（cache\_logs）留档。
        `function Install-Framework($reg) {`,
        `  $proc = Start-Process -FilePath ${ps(nodePath)} -ArgumentList @(${ps(npmCliJs)}, 'install', '@deepseek-ai/dsh@${target}', '--no-audit', '--no-fund', '--registry', $reg, '--fetch-timeout=45000', '--fetch-retries=1', '--fetch-retry-mintimeout=5000') -PassThru -WindowStyle Hidden`,

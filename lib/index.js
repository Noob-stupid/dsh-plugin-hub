  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `  try { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if (-not $c) { $bt = ''; try { $bt = (Resolve-Path (Join-Path ${ps(join(dirname(dshDir), '@deepseek-ai', 'dsh'))} 'lib\bin.js') -ErrorAction Stop).Path } catch {}; if ($bt -ne '') { Start-Process -FilePath ${ps(nodePath)} -ArgumentList $bt,'web' -WindowStyle Hidden } } } catch {}`,
        ...
        `  $proc = $null`,
        `  try {`,
        `    # pnpm 通道（事故教训）：npm-cli.js 在 schtasks 环境启动即卡死（0 字节日志、连网络请求都发不出）；`,
        `    # corepack pnpm 实测秒启动、16.9s 装完 rc.8（实测 exit=0 version=rc.8，含 node-pty/koffi 原生模块）。`,
        `    # dangerouslyAllowAllBuilds：pnpm 默认忽略依赖构建脚本并以 ERR_PNPM_IGNORED_BUILDS 报错退出。`,
        `    $cmdLine = 'title DSH-Upgrade && ${nodePath} ${corepackJs} pnpm install @deepseek-ai/dsh@${target} --force --config.dangerouslyAllowAllBuilds=true --registry ' + $reg + ' > "' + $npmOut + '" 2>&1'`,
        ...
        `  if (-not $ok -and -not $started) { $binNow = ''; try { $binNow = (Resolve-Path (Join-Path ${ps(join(dirname(dshDir), '@deepseek-ai', 'dsh'))} 'lib\bin.js') -ErrorAction Stop).Path } catch {}; if ($binNow -ne '') { Start-Process -FilePath ${ps(nodePath)} -ArgumentList $binNow,'web' -WindowStyle Hidden; $started = $true; Log '已发起拉起服务' } else { Log 'bin.js 缺失，跳过拉起' } }`,

 ...
 * api.github.com 在黑洞期单连接卡死：先主通道，再按序竞速镜像；
 * 每跳 15 秒超时，总失败时长控制在 45 秒内，避免用户长时间无反馈。
 */
async function githubRequest(url, options) {
  const candidates = url.startsWith(GITHUB_API)
    ? [url, ...API_MIRROR_PREFIXES.map((prefix) => `${prefix}${url.slice(GITHUB_API.length)}`)]
    : url.startsWith(GITHUB_RAW)
      ? [url, ...RAW_MIRROR_PREFIXES.map((prefix) => `${prefix}${url.slice(GITHUB_RAW.length)}`)]
      : [url]
  let last
  for (const candidate of candidates) {
    try {
      const res = await githubRequestOnce(candidate, { ...options, timeout: 15000 })
      // 非 2xx（含镜像返回的 403/404）不视为成功，继续下一个候选
  ...
  if (last?.res !== undefined) return last.res
  throw last?.error ?? new Error('GitHub 请求失败')
}
...
async function githubJson(url, signal, token = null) {
  ...
  try {
    ...
    // https 主通道与镜像全灭时，切 gh CLI 通道（本机 gh 认证可用）
    if (apiPath !== null) return githubViaGh(apiPath)
    throw error
  }
}
...
/** raw 文件读取：https 主通道（含镜像）→ gh CLI contents 兜底（黑洞期可用）。 */
async function rawTextWithFallback(repo, branch, path) {
  try {
    const body = await githubText(`${GITHUB_RAW}/${repo}/${encodeURIComponent(branch)}/${path}`)
    if (body !== null) return body
  } catch {}
  try {
    const data = await githubViaGh(`repos/${repo}/contents/${path}?ref=${encodeURIComponent(branch)}`)
...
async function aiRepair(job, ctx, profileDir, candidates, lastError) {
  ...
  const prompt = [
    ...
    '4) 执行 git/pnpm 前先设置环境变量 GIT_TERMINAL_PROMPT=0 和 GCM_INTERACTIVE=never，禁止弹出任何登录/凭据窗口；',
    '5) 不要杀进程、不要重启服务、不要改动与本次安装无关的文件。',

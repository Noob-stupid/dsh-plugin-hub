  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `Write-Host '升级框架本体 ${current} -> ${latest}…'`,
        `& ${JSON.stringify(nodePath)} ${JSON.stringify(npmCli)} install '@deepseek-ai/dsh@${latest}' --no-audit --no-fund --registry https://registry.npmmirror.com`,
        'if ($LASTEXITCODE -ne 0) {',
        `  Write-Host '升级失败，回滚框架本体…'`,
        ...
        '}',
        officialList !== '' ? `Write-Host '更新官方配套包…'\n  if (Test-Path ${JSON.stringify(corepackJs)}) { & ${JSON.stringify(nodePath)} ${JSON.stringify(corepackJs)} pnpm update ${officialList} --no-optional }` : '',
        'Write-Host \'拉起 DSH 服务…\'',
        '$ok = $false',
      ...
      upgraded = true
      steps.push(`框架升级脚本已启动：${current} → ${latest}（服务将自动停止→升级→回滚保护→拉起，约 1-3 分钟，期间页面会断开）`)
    ...
    sendJson(res, 200, { ok: true, current, latest, hasUpdate, upgraded, backupDir, steps, hints })
    return
  }

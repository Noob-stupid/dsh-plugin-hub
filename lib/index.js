  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `    # corepack pnpm 实测 372ms 秒启动、输出正常。用 pnpm + 国内源替代 npm。`,
        `    $cmdLine = 'title DSH-Upgrade && ${nodePath} ${corepackJs} pnpm install @deepseek-ai/dsh@${target} --force --registry ' + $reg + ' > "' + $npmOut + '" 2>&1'`,

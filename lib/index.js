...
async function detectCompat(baseUrl) {
  ...
  if (result.webAppVersion !== null && !SUPPORTED_WEB_APP_PATTERN.test(result.webAppVersion)) {
    ...
    result.notice = `当前 DSH web 包版本 ${result.webAppVersion} 不在受支持的 0.1.0 系列内，插件控制台的部分功能可能因官方破坏性更新而失效；请到 https://github.com/Noob-stupid/dsh-plugin-console 获取匹配的更新`
  }
  return result
}
...
function githubRequestOnce(url, { signal, accept } = {}) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject)
    req.setTimeout(60000, () => req.destroy(new Error('GitHub 请求超时')))
    if (signal !== undefined) {
      if (signal.aborted) {
...
async function githubJson(url, signal) {
  const res = await githubRequest(url, { signal })
  const status = res.statusCode ?? 0
  if (status === 403 && Number(res.headers['x-ratelimit-remaining'] ?? '1') === 0) {
    throw new Error('GitHub 匿名接口限流已用尽，请稍后再试')
  }
...
async function githubText(url, signal) {
  const res = await githubRequest(url, { signal, accept: 'application/json' })
  const status = res.statusCode ?? 0
  const body = await collectBody(res)
...
async function npmInstall(profileDir, spec) {
  const npm = process.platform === 'win32' ? 'npm.cmd' : 'npm'
  await execFileAsync(npm, ['install', '--prefix', profileDir, '--no-save', '--no-audit', '--no-fund', spec], {
    cwd: profileDir,
    timeout: 240000,
    windowsHide: true,
...
async function handle(ctx, req, res) {
  ...
  if (method === 'GET' && pathname === `${ROUTE_PREFIX}/state`) {
    ...
    sendJson(res, 200, { ok: true, entries, patchPath, compat, patch: { disables: patch.disables, forced: patch.forced, inserts: patch.inserts } })
    return
  }
  ...
  if (pathname === `${ROUTE_PREFIX}/repo`) {
    ...
    const meta = await githubJson(`${GITHUB_API}/repos/${repo}`, req.signal)
  ...
  if (pathname === `${ROUTE_PREFIX}/install`) {
    ...
    if (givenName !== '') {
      if (!npmNamePattern.test(givenName) || givenName.length > 214) {
        sendError(res, 400, 'packageName 不是合法的 npm 包名')
        return
      }
      packageName = givenName
    } else {
      // 兜底：宿主端自行拉取仓库元数据（慢网络下可能很慢，但保证可用）
      const meta = await githubJson(`${GITHUB_API}/repos/${repo}`, req.signal)
      const branch = meta.default_branch ?? 'main'
      const pkg = await fetchRepoPackage(repo, branch)
      if (pkg === null) {
        sendError(res, 400, `仓库 ${repo} 没有可用的 package.json，无法作为 npm 包安装`)
        return
      }
    ...
    const entryId = deriveEntryId(packageName, taken)
    const profileDir = dirname(patchPath)
    let installed = false
    try {
      await npmInstall(profileDir, packageName)
      installed = true
    } catch (registryError) {
      try {
        await npmInstall(profileDir, `github:${repo}`)
        installed = true
      } catch {
        sendError(res, 500, `安装失败：npm 注册表安装 ${packageName} 失败（${registryError.message}），且 git 安装 github:${repo} 也失败；请检查网络或包是否已发布`)
        return
      }
    }
    if (!installed) return
    const result = await appendInsert(patchPath, entryId, packageName)
    sendJson(res, 200, {
      ok: true,
      repo,
      entryId,
      ...
      patchPath,
      note: '已安装并写入启用条目，HMR 正在生效；若插件带界面，请刷新页面',
    })
    return
  }

...
async function handle(ctx, req, res) {
  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      // 升级脚本直接跑 npm 的 lib/cli.js（绕过 npm-cli.js）——事故教训：schtasks 任务环境下
      // npm-cli.js 的 spawn(cli.js) 环节会无声卡死（进程存活、0 字节日志、无 cli.js 子进程），
      // 而直跑 cli.js 完全正常（实测 install --dry-run exit 0）
      const npmCliJs = join(dirname(process.execPath), 'node_modules', 'npm', 'lib', 'cli.js')
      const corepackJs = join(dirname(process.execPath), 'node_modules', 'corepack', 'dist', 'corepack.js')
      // 预检（事故教训）：npm cli.js / dsh bin.js 缺失时**拒绝生成升级脚本**（不 kill 服务）——
      // 避免"kill 后升级/拉起双双失败，服务永久 down"。注意脚本实际跑 lib/cli.js（绕过
      // npm-cli.js 的 spawn 卡死），所以两个都要存在
      if (!existsSync(npmCli) || !existsSync(npmCliJs) || binPath === null || !existsSync(binPath)) {
        steps.push(`升级已取消：无法定位 ${!existsSync(npmCli) || !existsSync(npmCliJs) ? 'npm cli.js' : 'dsh bin.js'}（框架安装可能不完整），服务保持运行；请先修复框架再升级`)
      ...
      const lines = [
        ...
        `  try { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if (-not $c) { Start-Process -FilePath ${ps(nodePath)} -ArgumentList ${ps(binPath)},'web' -WindowStyle Hidden } } catch {}`,
        ...
        "SetState 'stopped' '停止服务…（页面将断开，稍后自动恢复）'",
        "Log '服务已停止'",
        'Start-Sleep -Seconds 3',
        `SetState 'installing' '升级框架 ${current} -> ${target}（npm 下载中）'`,
        // npm 前确保服务确实已停：升级断连期间用户可能手动拉起服务——它会锁住 dsh 目录，
        // npm 替换时 EBUSY 卡死（曾卡在 stopped 阶段）。发现端口有监听则再次停止。
        `try { $svc = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if ($svc) { $svc | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue }; Start-Sleep -Seconds 2; Log '检测到服务被手动拉起，已再次停止' } } catch {}`,
        ...
        // npm 安装函数：Start-Process 直跑 cli.js（绕过 npm-cli.js 的 spawn 卡死）。
        // 注意：**不能加 -RedirectStandardOutput/-RedirectStandardError**——schtasks 任务环境下
        // 带重定向的 Start-Process 子进程会启动即异常退出（ExitCode 读不到、输出 0 字节，实测）；
        // 无重定向时 ExitCode 正常（实测 exit=0）。npm 自身 debug 日志（cache\_logs）留档。
        // 超时策略：总时长 10 分钟硬上限 + npm debug 日志 5 分钟无更新判定卡死（网络黑洞/挂起时
        // 快速失败换 registry，不用干等 10 分钟）；等待期间每 15 秒更新进度状态（客户端可见）。
        // 成功后校验 package.json 版本 === target（防 npm 假成功）。
        `function Install-Framework($reg) {`,
        `  $startAt = Get-Date`,
        `  $deadline = $startAt.AddMinutes(10)`,
        ...
        `  $proc = $null`,
        `  try {`,
        `    $proc = Start-Process -FilePath ${ps(nodePath)} -ArgumentList @(${ps(npmCliJs)}, 'install', '@deepseek-ai/dsh@${target}', '--no-audit', '--no-fund', '--no-save', '--registry', $reg, '--fetch-timeout=45000', '--fetch-retries=1', '--fetch-retry-mintimeout=5000') -PassThru -WindowStyle Hidden`,
        `  } catch {`,
        `    Log ('启动 npm 失败：' + $_.Exception.Message)`,
        `    return 1`,
        `  }`,
        ...
        `      Log 'npm 超过 10 分钟未完成，已强制终止'`,
        `      return 1`,
        `    }`,
        `    if (-not $proc.HasExited -and $cacheDir -ne '') {`,
        `      $latestLog = Get-ChildItem (Join-Path $cacheDir '_logs') -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1`,
        `      if ($latestLog -and $latestLog.LastWriteTime -gt $lastLogM) { $lastLogM = $latestLog.LastWriteTime; $stallDeadline = (Get-Date).AddMinutes(5) }`,
        `      elseif ((Get-Date) -gt $stallDeadline) {`,
        `        Stop-Process -Id $proc.Id -Force -ErrorAction SilentlyContinue`,
        `        Log 'npm 超过 5 分钟无进度（debug 日志无更新），判定卡死，已强制终止'`,
        `        return 1`,
        `      }`,
        `    }`,
        `    if (-not $proc.HasExited) {`,
        `      SetState 'installing' ('升级框架 ${current} -> ${target}（npm 下载中，已等待 ' + [int]((Get-Date) - $startAt).TotalSeconds + ' 秒）')`,
        `      Start-Sleep -Seconds 15`,
        `    }`,
        `  }`,
        `  $code = 1`,
        `  try { $code = $proc.ExitCode } catch { Log ('读取 npm 退出码失败：' + $_.Exception.Message) }`,
        `  if ($code -eq 0) {`,
        `    try {`,
        `      $v = (Get-Content ${ps(join(dshDir, 'package.json'))} -Raw | ConvertFrom-Json).version`,
        `      if ($v -ne '${target}') { Log ('npm 退出码 0 但版本未更新（当前 ' + $v + '），按失败处理'); $code = 1 }`,
        ...
        `  if (Test-Path ${ps(join(dshDir, 'lib', 'bin.js'))}) { Log '回滚完成（框架目录已恢复）' } else { Log '回滚失败：复制失败，请手动修复框架安装' }`,
        `}`,
        ...
        "SetState 'relaunching' '拉起 DSH 服务…'",
        "Log '拉起 DSH 服务…'",
        '$ok = $false',
        '$started = $false',
        `for ($i = 0; $i -lt 20; $i++) {`,
        `  try { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if ($c.Count -gt 0) { $ok = $true; break } } catch {}`,
        `  if (-not $ok -and -not $started) { if (Test-Path ${ps(binPath)}) { Start-Process -FilePath ${ps(nodePath)} -ArgumentList ${ps(binPath)},'web' -WindowStyle Hidden; $started = $true; Log '已发起拉起服务' } else { Log 'bin.js 缺失，跳过拉起' } }`,
        '  Start-Sleep -Seconds 5',
        '}',
        `if ($ok) { SetState 'done' '升级完成，服务已监听 ${port}' ; Log '服务已监听 ${port}，升级完成' } else { SetState 'failed' '拉起失败：请手动运行 node ${binPath} web' ; Log '拉起失败：请手动运行 node ${binPath} web（参考本日志排查）' }`,
        `schtasks /delete /f /tn ${taskName} 2>$null`,
      ].filter((l) => l !== '').join('\r\n')
      // 初始状态（备份完成、脚本即将执行）
    ...
    const hints = [
      ...
      '重要：升级全程约 1-3 分钟，期间页面会断开且服务自动拉起——请勿手动拉起服务或重启桌面端，否则会中断升级（升级脚本可能被清理，npm 安装会被打断）。',
      ...steps,

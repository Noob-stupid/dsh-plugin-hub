...
function packageEntryExists(dir, pkg) {
  ...
  if (pkg.bin && typeof pkg.bin === 'object') candidates.push(...Object.values(pkg.bin).filter((v) => typeof v === 'string'))
    ...
    for (const sub of subs) {
      ...
      if (pkg && typeof pkg.name === 'string' && pkg.dsh?.bundle) {
        ...
        if (packageEntryExists(subDir, pkg)) {
          ...
          try {
            await installBundleFromRelease(subRepo, pkg.name, target)
            installed = true
          } catch (error) {
            report.push({ component: sub.name, type: 'bundle', ok: false, note: `无构建产物且 Release 拉取失败：${error.message}（可手动：${sub.url}/releases）` })
          }
        }
        if (installed) {
          try {
            await addBundleToManifest(profileDir, pkg.name)
            report.push({ component: sub.name, type: 'bundle', ok: true, note: `已装配 ${pkg.name}（bundle 层，需重启生效）` })
          } catch (error) {
            report.push({ component: sub.name, type: 'bundle', ok: false, note: `装配失败：${error.message}` })
          }
        }
        handled = true
      }
...
async function handle(ctx, req, res) {
  ...
  if (pathname === `${ROUTE_PREFIX}/check-update`) {
    ...
    sendJson(res, 200, { ok: true, packageName, latest, depsOutdated, error })
    return
  }

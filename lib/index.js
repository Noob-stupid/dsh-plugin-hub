...
async function handle(ctx, req, res) {
  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const launchSnippet = (tag) => `if (-not $ok -and -not $started) { if (Invoke-DshRelaunch '${tag}') { $started = $true } }`
      const lines = [
        `$state = ${ps(stateFile)}`,
        `$log = ${ps(logFile)}`,
        ...
        "  try { if ($s -ne 'done' -and $s -ne 'idle') { $script:stage = $s }; Set-Content -Path $state -Value ($s + '|' + $m) -Encoding UTF8 } catch {}",
        "}",
        ...
        `  if (-not $ok -and -not $started) { if (Invoke-DshRelaunch '升级后') { $started = $true } }`,
        '  Start-Sleep -Seconds 5',
        '}',
        ...
        `  for ($i = 0; $i -lt 20; $i++) {`,
        `    try { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if ($c.Count -gt 0) { $ok = $true; break } } catch {}`,
        `    if (-not $ok -and -not $started) { if (Invoke-DshRelaunch '回滚后') { $started = $true } }`,
        `    Start-Sleep -Seconds 5`,
        `  }`,
        `}`,

...
/** 占位符：${home} 家目录 / ${profile} DSH profile 目录 / ${python} Python 解释器 / ${scripts} Python Scripts / ${node} node 可执行。 */
...
async function aiEmpowerPlan(job, ctx, profileDir) {
  ...
  try {
    ...
    const outcome = await Promise.race([
      ...
      new Promise((resolve) => setTimeout(() => {
        controller.abort()
        resolve({ status: 'failed', detail: 'AI 赋能规划超时（10 分钟）' })
      }, 600000)),
    ])
    if (outcome.status === 'completed') {
...
async function compStatus(id) {
  const record = compFind(id)
  if (!record) throw new Error('组件不存在')
  const running = Boolean(record.pid && processAlive(record.pid))
  let healthy = null
  if (record.healthUrl && running) {
    try {

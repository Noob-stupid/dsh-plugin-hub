...
async function handle(ctx, req, res) {
  ...
  if (method === 'GET' && pathname === `${ROUTE_PREFIX}/framework-upgrade-status`) {
    ...
    try {
      ...
      if (existsSync(f)) {
        const [st, ...rest] = readFileSync(f, 'utf8').split('|')
        status = { status: st ?? 'idle', message: rest.join('|') || null, at: statSync(f).mtimeMs }
      }
    } catch {}
  ...
  if (method === 'POST' && pathname === `${ROUTE_PREFIX}/framework-relaunch`) {
    ...
    try {
      const ps1 = join(tmpdir(), `dsh-relaunch-${Date.now()}.ps1`)
      const lines = [
        '$ok = $false',
  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `& ${ps(nodePath)} ${ps(npmCli)} install '@deepseek-ai/dsh@${target}' --no-audit --no-fund --registry https://registry.npmmirror.com`,
        'if ($LASTEXITCODE -ne 0) {',
        "  SetState 'rollback' '升级失败，回滚框架本体…'",
        "  Log '升级失败，回滚框架本体…'",
        `  Remove-Item ${ps(dshDir)} -Recurse -Force -ErrorAction SilentlyContinue`,
        `  Copy-Item ${ps(rollbackDir)} ${ps(dshDir)} -Recurse -Force`,
        "  Log '回滚完成'",
        '} else { Log \'框架本体升级完成\' }',
      ...
      writeFile(ps1, `\uFEFF${lines}`, 'utf8').then(
        () => {
          ...
          const tr = `"powershell -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File \\"${ps1Posix}\\""`
          execFile('schtasks.exe', ['/create', '/f', '/tn', taskName, '/tr', tr, '/sc', 'once', '/st', '00:00', '/rl', 'highest'], { windowsHide: true }, (error) => {
            if (error) {

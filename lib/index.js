...
async function runInstallJob(job, ctx) {
  try {
    ...
    let installed = false
    try {
      await npmInstall(profileDir, job.packageName)
      installed = true
    } catch (registryError) {
      ...
      if (!installed) {
        job.status = 'failed'
        job.error = `安装失败：npm 镜像安装 ${job.packageName} 失败（${registryError?.message ?? '未知错误'}），且 git 安装（ghproxy + github 直连）也失败（${job.lastError?.message ?? '未知错误'}）；请检查网络或包是否已发布`
        return
      }
    }

  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `  $proc = Start-Process -FilePath ${ps(nodePath)} -ArgumentList @(${ps(npmCli)}, 'install', '@deepseek-ai/dsh@${target}', '--no-audit', '--no-fund', '--registry', $reg, '--fetch-timeout=45000', '--fetch-retries=1', '--fetch-retry-mintimeout=5000') -RedirectStandardOutput $npmOut -RedirectStandardError ($npmOut + '.err') -PassThru -WindowStyle Hidden`,

      ...
      if (ok) {
        ...
        try {
          ...
          if (!userPatch.includes(`id: ${ref.id}`)) {
            appendFileSync(userPatchPath, `- id: ${ref.id}\n  disabled: true\n`, 'utf8')
      ...
      if (installedName === null) {
        ...
        if (!subpackageMode && !expanded) {
          try {
            ...
            installedName = name
            // 聚合包依赖补装（事故教训）：dsh-web-ui-all 更新后 17 个捆绑依赖缺失，
            // cordis.patch.yml 引用未装包 → 服务加载崩溃。curl 只解压主包，这里逐个补装直接依赖。
            if (info.missingDeps.length > 0) {
              const stillMissing = []
              for (const dep of info.missingDeps) {
                ...
                if (!ok) {
                  try {
                    const depInfo = await curlManualInstall(profileDir, dep, registries)
                    ok = depInfo.missingDeps.length === 0
                  } catch {}
                }
              ...
              job.curlNote = `已通过 curl 通道安装 v${info.version}`
            }
          } catch (curlError) {
            lastError = curlError
          }

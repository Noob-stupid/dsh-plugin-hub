...
async function handle(ctx, req, res) {
  ...
  if (pathname === `${ROUTE_PREFIX}/toggle`) {
    ...
    if (enabled) {
      // 框架升级适配门（事故教训）：兼容门清单内的行保持强制禁用，直到更新并通过校验
      const compatPending = readCompatPending()
      const pend = (compatPending?.pending ?? []).find((p) => p.rowId === rowId && (p.status ?? 'pending') === 'pending')
      if (pend) {
        sendError(res, 409, `「${rowId}」仍在框架升级适配门中（${compatPending.frameworkVersion ?? '?'}）被强制禁用：请先点击「更新并适配 / 已适配，立即解锁」，通过兼容校验后系统自动解锁`)
        return
      }
    }
    const patchPath = findPatchPath(ctx)
  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      if (!existsSync(corepackJs) || binPath === null || !existsSync(binPath)) {
        ...
        sendJson(res, 200, { ok: true, current, latest, next, target, hasUpdate, upgraded: false, backupDir, configMigrations, steps, hints: steps })
        return
      }
      ...
      const lines = [
        ...
        '  Start-Sleep -Seconds 5',
        '}',
        // 拉起失败不再只留提示（2026-09-04 教训）：自动全树回滚并再拉起一次
    ...
    sendJson(res, 200, { ok: true, current, latest, next, target, hasUpdate, upgraded, backupDir, registryError, configMigrations, steps, hints })
    return
  }

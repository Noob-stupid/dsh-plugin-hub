  ...
  const walk = (dir, depth) => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        for (const sym of ['settingsNamespace', 'installSettingsSection']) {
          if (text.includes(sym) && !found.includes(sym)) found.push(sym)
        }
      } catch {}
    }
  }
...
function mergeQuarantineRecord() {
  ...
  for (const rowId of Array.isArray(rec.rows) ? rec.rows : []) {
...
const COMPONENTS_FILE = join(homedir(), '.dsh', 'plugin-console', 'components.json')
/** AI 赋能任务持久化文件（DSH 重启不丢计划/结果；running 任务重启后标记中断）。 */
const AI_JOBS_FILE = join(homedir(), '.dsh', 'plugin-console', 'ai-jobs.json')
/** 仓库落地根目录（可配置，默认 ~/.dsh/repos）。 */
let reposDirCache = null
const REPO_LAND_CONF = join(homedir(), '.dsh', 'plugin-console', 'repo-land.json')
function getReposDir() {
  if (reposDirCache !== null) return reposDirCache
  try {
    if (existsSync(REPO_LAND_CONF)) {
      const conf = JSON.parse(readFileSync(REPO_LAND_CONF, 'utf8'))
...
function setReposDir(dir) {
  ...
  mkdirSync(dirname(REPO_LAND_CONF), { recursive: true })
  writeFileSync(REPO_LAND_CONF, JSON.stringify({ dir: reposDirCache }, null, 2), 'utf8')
  return reposDirCache
}
...
function saveAiJobs() {
  try {
    mkdirSync(dirname(AI_JOBS_FILE), { recursive: true })
    ...
    writeFileSync(AI_JOBS_FILE, JSON.stringify(arr, null, 2), 'utf8')
  } catch {}
}
function loadAiJobs() {
  try {
    if (!existsSync(AI_JOBS_FILE)) return
    const arr = JSON.parse(readFileSync(AI_JOBS_FILE, 'utf8'))
    for (const j of arr) {
...
function findComponents() {
  try {
    const parsed = JSON.parse(readFileSync(COMPONENTS_FILE, 'utf8'))
...
function saveComponents(list) {
  mkdirSync(dirname(COMPONENTS_FILE), { recursive: true })
  writeFileSync(COMPONENTS_FILE, JSON.stringify(list, null, 2), 'utf8')
}
...
export function apply(ctx) {
  ...
  try { mergeQuarantineRecord() } catch {}
...
async function handle(ctx, req, res) {
  ...
  if (method === 'GET' && pathname === `${ROUTE_PREFIX}/state`) {
    ...
    try {
      ...
      if (rr !== null && typeof rr.checkpointDir === 'string' && existsSync(join(rr.checkpointDir, '.pnpm'))) {
        rollbackRec = { from: rr.from ?? null, to: rr.to ?? null, at: rr.at ?? null }
      }
    } catch {}
    const entries = listEntries(ctx).map((entry) => {
      ...
      return {
        ...
        pendingCompat: compatPendingRows.has(entry.rowId),
  ...
  if (pathname === `${ROUTE_PREFIX}/adapt-unlock-all`) {
    ...
    const patchPath = findPatchPath(ctx)
    const profileDir = dirname(patchPath)
    const targets = (pending?.pending ?? []).filter((p) => (p.status ?? 'pending') === 'pending' && (p.moduleName === root || String(p.moduleName).startsWith(root + '/')))
    if (targets.length === 0) {
      // 无待适配行=全部已适配/已解锁，属正常状态：返回友好提示而非错误（点击「一键启用已适配」不应报"操作失败"）
      sendJson(res, 200, { ok: true, unlocked: [], kept: [], note: '该全家桶没有待适配行（已全部适配/解锁），无需操作' })
      return
    }
    const fwVer = typeof pending.frameworkVersion === 'string' ? pending.frameworkVersion : '?'
    const unlocked = []
    ...
    writeCompatPending(pending)
    sendJson(res, 200, { ok: true, root, unlocked, kept })
    return
  }
  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      // 拉起服务的 PowerShell 片段（隔离重试用；与升级/回滚路径同一套解析逻辑 + 输出落 fw-relaunch.log）
      const launchSnippet = (tag) => `if (-not $ok -and -not $started) { $binNow = ''; try { $env:DSH_RESOLVE_ROOT = '${join(dirname(fileURLToPath(import.meta.url)), '..')}'; $binNow = (& ${ps(nodePath)} -e "const path=require('path');const p=require.resolve('@deepseek-ai/dsh/package.json',{paths:[process.env.DSH_RESOLVE_ROOT]});console.log(path.join(path.dirname(p),'lib','bin.js'))" 2>$null | Select-Object -Last 1) } catch {}; if ($binNow -ne '' -and (Test-Path $binNow)) { $launchLog = Join-Path (Split-Path $log) 'fw-relaunch.log'; try { Add-Content -Path $launchLog -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' 拉起(${tag}): ' + $binNow) -Encoding UTF8 } catch {}; Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', ('"' + ${ps(nodePath)} + '" "' + $binNow + '" web >> "' + $launchLog + '" 2>&1')) -WindowStyle Hidden; $started = $true; Log '已发起拉起服务（${tag}，输出见 fw-relaunch.log）' } else { Log 'bin.js 缺失，跳过拉起' } }`
      const lines = [
        `$state = ${ps(stateFile)}`,
        `$log = ${ps(logFile)}`,
        ...
        `  try { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if (-not $c) { $bt = ''; try { $env:DSH_RESOLVE_ROOT = '${join(dirname(fileURLToPath(import.meta.url)), '..')}'; $bt = (& ${ps(nodePath)} -e "const path=require('path');const p=require.resolve('@deepseek-ai/dsh/package.json',{paths:[process.env.DSH_RESOLVE_ROOT]});console.log(path.join(path.dirname(p),'lib','bin.js'))" 2>$null | Select-Object -Last 1) } catch {}; if ($bt -ne '') { $launchLog = Join-Path (Split-Path $log) 'fw-relaunch.log'; try { Add-Content -Path $launchLog -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' 拉起(异常兜底): ' + $bt) -Encoding UTF8 } catch {}; Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', ('"' + ${ps(nodePath)} + '" "' + $bt + '" web >> "' + $launchLog + '" 2>&1')) -WindowStyle Hidden } } } catch {}`,
        ...
        `  if (-not $ok -and -not $started) { $binNow = ''; try { $env:DSH_RESOLVE_ROOT = '${join(dirname(fileURLToPath(import.meta.url)), '..')}'; $binNow = (& ${ps(nodePath)} -e "const path=require('path');const p=require.resolve('@deepseek-ai/dsh/package.json',{paths:[process.env.DSH_RESOLVE_ROOT]});console.log(path.join(path.dirname(p),'lib','bin.js'))" 2>$null | Select-Object -Last 1) } catch {}; if ($binNow -ne '' -and (Test-Path $binNow)) { $launchLog = Join-Path (Split-Path $log) 'fw-relaunch.log'; try { Add-Content -Path $launchLog -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' 拉起(升级后): ' + $binNow) -Encoding UTF8 } catch {}; Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', ('"' + ${ps(nodePath)} + '" "' + $binNow + '" web >> "' + $launchLog + '" 2>&1')) -WindowStyle Hidden; $started = $true; Log '已发起拉起服务（输出见 fw-relaunch.log）' } else { Log 'bin.js 缺失，跳过拉起' } }`,
        '  Start-Sleep -Seconds 5',
        '}',
        ...
        `  for ($i = 0; $i -lt 20; $i++) {`,
        `    try { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if ($c.Count -gt 0) { $ok = $true; break } } catch {}`,
        `    if (-not $ok -and -not $started) { $binNow = ''; try { $env:DSH_RESOLVE_ROOT = '${join(dirname(fileURLToPath(import.meta.url)), '..')}'; $binNow = (& ${ps(nodePath)} -e "const path=require('path');const p=require.resolve('@deepseek-ai/dsh/package.json',{paths:[process.env.DSH_RESOLVE_ROOT]});console.log(path.join(path.dirname(p),'lib','bin.js'))" 2>$null | Select-Object -Last 1) } catch {}; if ($binNow -ne '' -and (Test-Path $binNow)) { $launchLog = Join-Path (Split-Path $log) 'fw-relaunch.log'; try { Add-Content -Path $launchLog -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' 拉起(回滚后): ' + $binNow) -Encoding UTF8 } catch {}; Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', ('"' + ${ps(nodePath)} + '" "' + $binNow + '" web >> "' + $launchLog + '" 2>&1')) -WindowStyle Hidden; $started = $true; Log '已发起拉起服务（回滚后，输出见 fw-relaunch.log）' } else { Log 'bin.js 缺失，跳过拉起' } }`,
        `    Start-Sleep -Seconds 5`,
        `  }`,
        `}`,

 ...
 * 2) 否则扫描依赖/peerDeps 的 @deepseek-ai/*：任一范围明确不含框架版本 → fail；
 * 3) 无声明且依赖全部满足 → unknown（新版变动不大；由调用方在「版本已变化」前提下降级为放行）。
 * 返回 { decision: 'pass'|'fail'|'unknown', reason }。
 */
function checkPluginFrameworkCompat(pkg, frameworkVersion) {
  const declared = pkg?.dsh?.engines?.framework ?? pkg?.dsh?.compat?.framework ?? pkg?.engines?.dsh ?? null
  if (typeof declared === 'string' && declared !== '') {
    return semverRangeMatchLoose(frameworkVersion, declared)
      ? { decision: 'pass', reason: `声明兼容范围 ${declared} 满足框架 ${frameworkVersion}` }
      : { decision: 'fail', reason: `声明兼容范围 ${declared} 不满足当前框架 ${frameworkVersion}` }
  }
...
async function maybeAutoAdaptCompat({ profileDir, packageName, syncedNames, patchPath, ctx }) {
  ...
  for (const p of targets) {
    ...
    try {
      const require = createRequire(join(profileDir, 'package.json'))
      const pkgPath = require.resolve(`${p.moduleName}/package.json`)
      pkg = JSON.parse(readFileSync(pkgPath, 'utf8'))
    } catch {}
    const version = typeof pkg?.version === 'string' ? pkg.version : null
    const changed = version !== null && version !== p.version
    const check = pkg !== null ? checkPluginFrameworkCompat(pkg, fwVer) : { decision: 'unknown', reason: '无法读取包信息' }
...
async function frameworkCompatReportFor(source, ctx, profileDir) {
  ...
  const installed = (() => {
    try {
      const require = createRequire(join(profileDir, 'package.json'))
      const pkgPath = require.resolve(`${norm}/package.json`)
...
async function handle(ctx, req, res) {
  ...
  if (pathname === `${ROUTE_PREFIX}/toggle`) {
    ...
    if (enabled) {
      ...
      if (pend) {
        sendError(res, 409, `「${rowId}」仍在框架升级适配门中（${compatPending.frameworkVersion ?? '?'}）被强制禁用：请先点击「更新并适配」，更新完成并通过兼容校验后系统自动解锁`)
        return
      }
    }
    const patchPath = findPatchPath(ctx)
  ...
  if (pathname === `${ROUTE_PREFIX}/check-update`) {
    ...
    sendJson(res, 200, { ok: true, packageName, latest, next, beta, depsOutdated, error, source })
    return
  }
  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `  if ($cp -ne '' -and (Test-Path (Join-Path $cp '.pnpm'))) {`,
        `    $restored = 0`,
        ...
        `  for ($i = 0; $i -lt 20; $i++) {`,
        `    try { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if ($c.Count -gt 0) { $ok = $true; break } } catch {}`,
        `    if (-not $ok -and -not $started) { $binNow = ''; try { $env:DSH_RESOLVE_ROOT = '${join(dirname(fileURLToPath(import.meta.url)), '..')}'; $binNow = (& ${ps(nodePath)} -e "const path=require('path');const p=require.resolve('@deepseek-ai/dsh/package.json',{paths:[process.env.DSH_RESOLVE_ROOT]});console.log(path.join(path.dirname(p),'lib','bin.js'))" 2>$null | Select-Object -Last 1) } catch {}; if ($binNow -ne '' -and (Test-Path $binNow)) { Start-Process -FilePath ${ps(nodePath)} -ArgumentList $binNow,'web' -WindowStyle Hidden; $started = $true; Log '已发起拉起服务（回滚后）' } else { Log 'bin.js 缺失，跳过拉起' } }`,
        `    Start-Sleep -Seconds 5`,
        `  }`,
        `}`,
  ...
  if (pathname === `${ROUTE_PREFIX}/repo-clone`) {
    // 仓库落地：从 GitHub 克隆任意项目到 <配置路径>/<owner>/<name>（不限于 DSH 插件）
    ...
    if (!m) {
      sendError(res, 400, '仓库格式不正确：请提供 owner/repo 或 GitHub 仓库链接')
      return
    }
    const [, owner, name] = m
    ...
    try {
      ...
      await execFileAsync('git.exe', ['clone', '--depth', '1', `https://github.com/${owner}/${name}.git`, target], { cwd: root, timeout: 600000, windowsHide: true, maxBuffer: 4 * 1024 * 1024 })
      sendJson(res, 200, { ok: true, owner, name, path: target })
    } catch (error) {

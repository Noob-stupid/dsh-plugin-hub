...
async function npmInstall(profileDir, spec) {
  const args = ['install', '--prefix', profileDir, '--no-save', '--no-audit', '--no-fund',
    '--registry', 'https://registry.npmmirror.com', spec]
  const opts = {
    cwd: profileDir,
    timeout: 240000,
    windowsHide: true,
  ...
  if (process.platform === 'win32') {
    // Windows 上 execFile('npm.cmd') 会 EINVAL（.cmd 批处理不能直接启动）。
    // 优先用 node 直跑 npm-cli.js（与 npm.cmd 内部完全一致，绕开批处理壳）。
    const npmCli = join(dirname(process.execPath), 'node_modules', 'npm', 'bin', 'npm-cli.js')
    if (existsSync(npmCli)) {
      await execFileAsync(process.execPath, [npmCli, ...args], opts)
      return
    }
    // 兜底：经 cmd.exe 运行 npm（引号由 cmd 语义处理）
    const cmd = `${JSON.stringify('npm')} ${args.map((arg) => JSON.stringify(arg)).join(' ')}`
    await execFileAsync(process.env.ComSpec ?? 'cmd.exe', ['/d', '/s', '/c', cmd], opts)
    return
  }
  await execFileAsync('npm', args, opts)
}
...
async function handle(ctx, req, res) {
  ...
  if (method === 'GET' && pathname === `${ROUTE_PREFIX}/state`) {
    ...
    sendJson(res, 200, { ok: true, entries, patchPath, compat, github: { loggedIn: auth.loggedIn, login: auth.login }, patch: { disables: patch.disables, forced: patch.forced, inserts: patch.inserts } })
    return
  }
  ...
  if (pathname === `${ROUTE_PREFIX}/install`) {
    ...
    try {
      ...
      const gitSpecs = [
        `git+https://ghproxy.net/https://github.com/${repo}.git`,
        `github:${repo}`,
      ]
      let lastGitError = null
      for (const spec of gitSpecs) {
        try {
          await npmInstall(profileDir, spec)
          installed = true
          break
        } catch (error) {
          lastGitError = error
      ...
      if (!installed) {
        sendError(res, 500, `安装失败：npm 镜像安装 ${packageName} 失败（${registryError.message}），且 git 安装（ghproxy + github 直连）也失败（${lastGitError?.message ?? '未知错误'}）；请检查网络或包是否已发布`)
        return
      }
    }
    if (!installed) return

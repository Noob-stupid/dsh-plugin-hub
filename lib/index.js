...
/** 纯 bundle 包判定：声明 dsh.bundle 且没有可加载 JS 入口（如皮肤/资产聚合包）。
 * 这类包不能作为插件条目挂载，必须按官方 `dsh plugin add` 的行为追加为 profile bundle 层。 */
async function detectBundleOnly(profileDir, packageName) {
  try {
    ...
    const entry = pkg.exports?.['.'] ?? pkg.main ?? null
    return entry == null
  } catch {
    return false
  }
...
async function runInstallJob(job, ctx) {
  try {
    ...
    if (job.packageName === null) {
      ...
      if (pkg.private === true) {
        job.status = 'failed'
        job.error = `仓库 ${job.repo} 的根包未发布到 npm（private: true），无法直接安装；请在市场"查看"详情里选择其子包安装，或直接搜索子包名`
        return
      }
    ...
    if (await detectBundleOnly(profileDir, job.packageName)) {
      // 官方 dsh plugin add 行为：纯 bundle 包（无 JS 入口）追加为 profile bundle 层，
      // 重启后其 cordis.patch.yml 作为 bundle 层参与组合
      await addBundleToManifest(profileDir, job.packageName)
      job.bundle = true
      job.status = 'done'
      return
    }

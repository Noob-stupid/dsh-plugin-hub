  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `      curl.exe -s -L -m 60 --insecure -o $tgz $url`,
        ...
        `  }`,
        `  if ($nameOk -and $clientOk) { Log ('自报名一致性 OK：' + $expectedName) } else { Log ('⚠️ 自报名一致性异常：部署目录=' + (Split-Path $selfDir -Leaf) + '，期望 ' + $expectedName + '（index=' + $nameOk + ' client=' + $clientOk + '）——请检查部署目录与包名是否匹配') }`,
        `}`,

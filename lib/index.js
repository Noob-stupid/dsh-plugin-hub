...
async function enrichItems(items) {
  return Promise.all(items.map(async (item) => {
    ...
    try {
      ...
      const [pkgResult, skillResult] = await Promise.allSettled([
        Promise.any([
          curlText(base, 4000),
    ...
    return { ...item, official, aggregate, aggregateInstallable, hasSkill }
  }))
}
...
async function handle(ctx, req, res) {
  ...
  if (pathname === `${ROUTE_PREFIX}/repo`) {
    ...
    const auth = readGithubAuth()
    // meta 降级策略：githubJson（https+镜像+gh）与 curl 竞速；全失败不阻塞——
    // 用默认分支 main 继续（raw 读取走 jsDelivr CDN 兜底，黑洞期也快），避免 40s+ 卡死
    let meta = null
    try {
  ...
  if (pathname === `${ROUTE_PREFIX}/install`) {
    ...
    const kind = body.kind === 'skill' ? 'skill' : 'plugin'
    if (kind === 'skill' && repo === '') {
      sendError(res, 400, '技能安装必须提供仓库（owner/name）')
      return
    }
    ...
    void (kind === 'skill' ? runSkillInstallJob(job, ctx) : runInstallJob(job, ctx))

  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `  $proc = $null`,
        `  try {`,
        `    $proc = Start-Process -FilePath ${ps(nodePath)} -ArgumentList @(${ps(npmCli)}, 'install', '@deepseek-ai/dsh@${target}', '--force', '--no-audit', '--no-fund', '--no-save', '--registry', $reg, '--fetch-timeout=45000', '--fetch-retries=1', '--fetch-retry-mintimeout=5000') -PassThru -WindowStyle Hidden`,
        `  } catch {`,

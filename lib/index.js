...
function builtinPlanFor(source) {
  if (!/openviking/i.test(source)) return null
  const home = homedir()
  ...
  const ovConf = {
    ...
    embedding: { dense: { provider: 'local', model: 'bge-small-zh-v1.5-f16', model_path: '${home}/.cache/openviking/models/bge-small-zh-v1.5-f16.gguf' } },
    storage: { workspace: storage },
  }
  const server = { name: 'openviking-server', file: '${python}', args: ['-m', 'openviking_cli.server_bootstrap'], cwd: '${home}/.openviking', healthUrl: 'http://127.0.0.1:1933/health', port: 1933 }
  return {
    type: 'service',
...
async function aiEmpowerPlan(job, ctx, profileDir) {
  ...
  try {
    ...
    if (outcome.status === 'completed') {
      const plan = parsePlanJson(String(outcome.detail ?? ''))
      if (plan === null) {
        job.status = 'failed'
...
async function runAiStep(job, step, ctx, profileDir, signal) {
  ...
  switch (step.action) {
    ...
    case 'install-npm': {
      const pkg = String(step.package ?? '')
      if (!/^(@[A-Za-z0-9_\-\.]+\/)?[A-Za-z0-9_\-\.]+$/u.test(pkg)) throw new Error(`包名不合法：${pkg}`)
    ...
    case 'write-file': {
      ...
      mkdirSync(dirname(target), { recursive: true })
      const content = resolvePlaceholders(String(step.content ?? ''), { profileDir: profile })
    ...
    case 'start-service': {
      ...
      if (healthUrl !== null) {
        try {
          ...
          if (probe.ok) {
            ...
            compUpsert({ id: slugPre || `svc-${Date.now()}`, name, kind: 'server', pid: null, file, args, cwd, healthUrl, port, repo: job.source, installedAt: Date.now() })
      ...
      compUpsert({ id, name, kind: 'server', pid, file, args, cwd, healthUrl, port, repo: job.source, installedAt: Date.now() })
    ...
    case 'register-component': {
      ...
      compUpsert({ id, name: String(step.name ?? id), kind: String(step.kind ?? 'server'), pid: null, file: step.file ? resolvePlaceholders(String(step.file), { profileDir: profile }) : undefined, args: (step.args ?? []).map((a) => resolvePlaceholders(String(a), { profileDir: profile })), healthUrl: step.healthUrl ? resolvePlaceholders(String(step.healthUrl), { profileDir: profile }) : undefined, port: Number(step.port ?? 0), repo: job.source, installedAt: Date.now() })
      return
    }
    default:
...
async function compStatus(id) {
  ...
  return { id, name: record.name, running, healthy, pid: record.pid ?? null, port: record.port ?? null }
}
...
async function handle(ctx, req, res) {
  ...
  if (method === 'GET' && pathname === `${ROUTE_PREFIX}/state`) {
    ...
    sendJson(res, 200, { ok: true, entries, patchPath, compat, installJobs: jobs, recentFailures, github: { loggedIn: auth.loggedIn, login: auth.login }, patch: { disables: patch.disables, forced: patch.forced, inserts: patch.inserts }, framework, selfVersion, components: findComponents().map((c) => ({ id: c.id, name: c.name, kind: c.kind ?? 'server', pid: c.pid ?? null, port: c.port ?? null, healthUrl: c.healthUrl ?? null })) })
    return
  }
  ...
  if (method === 'GET' && pathname === `${ROUTE_PREFIX}/components`) {
    const list = findComponents()
    const enriched = []
    for (const c of list) {
      try {
        ...
        enriched.push({ id: c.id, name: c.name, running: false, healthy: null, pid: null, port: c.port ?? null })
      }
    }
    sendJson(res, 200, { ok: true, components: enriched })
    return
  }

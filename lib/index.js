  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `  $proc = $null`,
        `  try {`,
        `    $cmdLine = 'title DSH-Upgrade && ${nodePath} ${npmCli} install @deepseek-ai/dsh@${target} --force --no-audit --no-fund --no-save --registry ' + $reg + ' --fetch-timeout=45000 --fetch-retries=1 --fetch-retry-mintimeout=5000 > "' + $npmOut + '" 2>&1'`,
        ...
        `      if ($latestLog -and $latestLog.Length -gt 0) { $logSeen = $true }`,
        `      if ($logSeen) {`,
        ...
        `      $tail = ''`,
        `      try { $last = Get-Content $npmOut -Tail 1 -ErrorAction SilentlyContinue; if ($last) { $tail = ($last -join ' ').Trim() } } catch {}`,
        `      $waitMsg = '升级框架 ${current} -> ${target}（已等待 ' + [int]((Get-Date) - $startAt).TotalSeconds + ' 秒'`,
        `      if ($tail -ne '') { $waitMsg = $waitMsg + '；' + $tail }`,
        `      SetState 'installing' ($waitMsg + '）')`,

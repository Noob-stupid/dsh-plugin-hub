...
/** 仓库落地根目录：仓库落地功能的 git clone 目标。 */
const REPOS_ROOT = join(homedir(), '.dsh', 'repos')
function saveAiJobs() {
  try {
...
async function handle(ctx, req, res) {
        ...
        (async () => {
          try {
            const data = await githubJson(
              `${GITHUB_API}/search/repositories?q=${encodeURIComponent(`${query} topic:dsh-plugin`)}&sort=stars&order=desc&per_page=20&page=${page}`,
              req.signal,
              auth.token,
            )
  ...
  if (pathname === `${ROUTE_PREFIX}/repo-clone`) {
    // 仓库落地：从 GitHub 克隆任意项目到 ~/.dsh/repos/<owner>/<name>（不限于 DSH 插件）
    ...
    const [, owner, name] = m
    const target = join(REPOS_ROOT, owner, name)
    ...
    try {
      ...
      await execFileAsync('git.exe', ['clone', '--depth', '1', `https://github.com/${owner}/${name}.git`, target], { cwd: REPOS_ROOT, timeout: 600000, windowsHide: true, maxBuffer: 4 * 1024 * 1024 })
  ...
  if (pathname === `${ROUTE_PREFIX}/repo-open`) {
    ...
    const rootNorm = REPOS_ROOT.replace(/[\\/]+$/u, '')

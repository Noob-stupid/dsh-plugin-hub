  ...
  if (pathname === `${ROUTE_PREFIX}/framework-upgrade`) {
    ...
    if (hasUpdate && dshDir !== null) {
      ...
      const lines = [
        ...
        `  if (Test-Path ${ps(join(dshDir, 'lib', 'bin.js'))}) { Log '回滚完成（框架目录已恢复）' } else { Log '回滚失败：备份缺失或复制失败，请手动修复框架安装' }`,
        `}`,
        ...
        officialList !== '' ? `SetState 'pkg' '更新官方配套包…'\n  Log '更新官方配套包…'\n  Set-Location -Path ${ps(profileDir2)}\n  if (Test-Path ${ps(corepackJs)}) { & ${ps(nodePath)} ${ps(corepackJs)} pnpm update ${officialList} --no-optional; Log ('官方配套包更新结束（exit=' + $LASTEXITCODE + '）') }` : '',
        ...
        `  if (-not $ok) { if (Test-Path ${ps(binPath)}) { Start-Process -FilePath ${ps(nodePath)} -ArgumentList ${ps(binPath)},'web' -WindowStyle Hidden } else { Log 'bin.js 缺失，跳过拉起' } }`,

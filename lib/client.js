window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 只取仓库的 npm 包名（跳过 README，供"添加到本地"快捷路径使用）。 */
		async function fetchPackageName(repo) {
			const meta = await githubFetch(`${GITHUB_API}/repos/${encodeURIComponent(repo)}`);
			const branch = meta.default_branch ?? "main";
			try {
				const raw = await fetch(`${GITHUB_RAW}/${encodeURIComponent(repo)}/${encodeURIComponent(branch)}/package.json`);
		...
		function PluginConsoleTab({ t }) {
			...
			const addLocal = (item) => {
				...
				fetchPackageName(item.fullName).then(
					installWith,
						...
						: market.data.map((item) => {
							...
							return el("li", { key: item.fullName, className: styles.item },
								...
								desc ? el("div", null,
									...
									long ? el("button", {
										type: "button",
										onClick: () => setExpandedDescs((prev) => ({ ...prev, [item.fullName]: !open })),
									}, open ? t("collapseDesc") : t("expandDesc")) : null,
								) : null,
								el("div", { className: styles.rowTop },
									el("button", {
										type: "button",

		...
		function PluginConsoleTab({ t }) {
			...
			return el("section", { className: styles.section, "aria-busy": state.status === "loading" },
				...
				market !== null && market.status === "ready" && market.data.length > 0
					? el("div", { className: styles.stickyTop },
						el("button", {
							type: "button",
							className: styles.toggle,
							onClick: () => {
					...
					? el("div", { className: styles.search },
						el("input", {
							...
							onBlur: () => {
								// 点附近自动关：输入框失焦且无内容时收起
								setTimeout(() => {
									if (installedQuery.trim() === "" && document.activeElement !== document.getElementById("pc-installed-search-input")) {

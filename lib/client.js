...
(error) => setMessage(t("failed") + "：" + friendlyGithubError(error).message),
				...
				el("div", { id: "pc-installed-search-area" },
					el("div", { className: styles.installedHead },
						...
						// 兼容门总开关（用户定案：自动行为必须可关，关掉即纯手动）
						state.status === "ready"
							? el("label", { className: styles.tag, title: t("compatGateHint"), style: { display: "inline-flex", alignItems: "center", gap: 4, cursor: "pointer", userSelect: "none" } },
								el("input", {
									type: "checkbox",
									checked: (state.data.compatGate ?? {}).autoDisable !== false,
									onChange: (event) => setCompatGate({ autoDisable: event.currentTarget.checked }),
								}),
								t("compatGateAutoDisable"))
							: null,
						state.status === "ready"
							? el("label", { className: styles.tag, title: t("compatGateHint"), style: { display: "inline-flex", alignItems: "center", gap: 4, cursor: "pointer", userSelect: "none" } },
								el("input", {
									type: "checkbox",
									checked: (state.data.compatGate ?? {}).autoDetect !== false,
									onChange: (event) => setCompatGate({ autoDetect: event.currentTarget.checked }),
								}),
								t("compatGateAutoDetect"))
							: null,
						mode === "plugins"

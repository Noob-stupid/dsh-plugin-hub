				...
				call("/plugin-console/market-index", {}).then(
					(data) => {
						if (data && Array.isArray(data.items)) {
							...
							// ★「只看官方」需要官方标记：只补标**首屏可见的一段**（每条要发网络请求，
							// 全量 500 条会打满浏览器连接数，导致打开插件页明显变慢）。
							const base = data.items;
							const ENRICH_LIMIT = 40;
							const head = base.slice(0, ENRICH_LIMIT);
							// 延后到首屏渲染之后再补标：打开插件页时不再被网络请求抢连接
							const runEnrich = () => enrichOfficialBundle(head).then((enriched) => {
								...
								const rest = base.slice(ENRICH_LIMIT);
								setIndexData((prev) => (prev !== null && prev.loaded === true ? { ...prev, items: [...merged, ...rest] } : prev));
							});

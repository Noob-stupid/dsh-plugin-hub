	...
	factory: (require) => {
		...
		async function enrichOfficialBundle(items) {
			return Promise.all(items.map(async (item) => {
				let official = null;
				let aggregate = false;
				const pkgText = await fetchRawText(item.fullName, item.defaultBranch, "package.json");
				if (pkgText !== null) {
					try {
						const pkg = JSON.parse(pkgText);

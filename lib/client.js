window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			resetSources: "恢复默认",
			closeModal: "关闭",
			invalidSourceUrl: "地址必须是 https:// 开头的合法 URL",
			sourcesUpdated: "软件源已更新",
			sourceOf: "源：",
			searchSourceTitle: "点击切换搜索源（GitHub / Gitee / GitLab）",
			recentFailures: "最近安装失败",
			installedAt: "已安装",
			update: "更新版本",
			officialBadge: "官方",
		...
		const en = {
			...
			closeModal: "Close",
			invalidSourceUrl: "URL must start with https://",
			sourcesUpdated: "Sources updated",
			sourceOf: "Source: ",
			searchSourceTitle: "Click to switch search source (GitHub / Gitee / GitLab)",
			recentFailures: "Recent install failures",
			...
			officialEmpty: "No official/aggregate plugins.",
		...
		async function fetchRawText(repo, branch, file, source = "github") {
			...
			if (source === "gitlab") {
				try {
					const res = await fetch(`https://gitlab.com/${repo}/-/raw/${branch}/${file}`, { signal: AbortSignal.timeout(15000) });
					if (!res.ok) return null;
					return await res.text();
				} catch { return null; }
			}
		...
		function PluginConsoleTab({ t }) {
			...
			const [searchSource, setSearchSource] = react.useState(() => {
				try { const s = localStorage.getItem("pc-search-source"); return s === "gitee" || s === "gitlab" ? s : "github"; } catch { return "github"; }
			});
			...
			const search = () => {
				...
				if (searchSource !== "github") {
					// Gitee / GitLab：走服务端平台检索（浏览器直连通道对这些平台不可靠）
			...
			const openSources = () => {
				...
				call("/plugin-console/sources").then(
					(data) => setSourcesData(data.sources),
			...
			const sourcesAction = (payload, done) => {
				...
				call("/plugin-console/sources", payload).then(
					(data) => { setSourcesData(data.sources); setMessage(t("sourcesUpdated")); if (done) done(); },
					(error) => setMessage(t("failed") + "：" + friendlyGithubError(error).message),
				).finally(() => setSourcesBusy(false));
			};
			const addSource = () => {
				if (!/^https:\/\/\S+$/u.test(sourceUrl.trim())) {
			...
			const startJob = (repo, packageName, source = "github") => {
				...
				setMessage(null);
				call("/plugin-console/install", { repo, packageName, source }).then(
					(data) => {
						if (data && data.jobId) {
							setJobs((prev) => ({
								...prev,
			...
			const addLocal = (item) => {
				...
				const installWith = (info) => {
					...
					if (info.private === true) {
						...
						setMessage(t("privateRootHint") + "；已自动尝试其子包安装…");
			...
			const marketShown = market === null || market.status !== "ready"
				? []
				: officialOnly
					? market.data.filter((item) => item.official === true || item.aggregate === true)
					: market.data;
					...
					: market.data.length === 0
						...
						: marketShown.length === 0
							? [el("p", { key: "m", className: styles.status },
								officialOnly && market.data.some((item) => item.official === undefined)
									? t("officialChecking")
									: officialOnly ? t("officialEmpty") : t("marketEmpty"))]
			...
			return el("section", { className: styles.section, "aria-busy": state.status === "loading" },
						...
						el("button", {
							...
							searchSource === "github"
								? (state.status === "ready" && state.data.github && state.data.github.loggedIn
									...
									: t("githubCornerOut"))
								: t("sourceOf") + (searchSource === "gitee" ? "Gitee" : "GitLab"),
							" ▾"),
						sourceMenuOpen
							? el("div", { className: styles.sourceMenu },
								["github", "gitee", "gitlab"].map((s) =>
									el("button", {
										type: "button",
										key: s,
										className: styles.sourceOpt,
										"data-active": searchSource === s ? "true" : "false",
										onClick: () => {
											setSearchSource(s);
											try { localStorage.setItem("pc-search-source", s); } catch {}
											setSourceMenuOpen(false);
										},
									}, s === "github" ? "GitHub" : s === "gitee" ? "Gitee" : "GitLab")))
							: null,
					)),
				el("button", {
					type: "button",
				...
				market !== null && market.status === "ready" && market.data.some((item) => item.official === undefined)
					? el("p", { className: styles.note }, t("officialChecking"))
					: null,
								...
								: el("div", { style: { flexDirection: "column", gap: "8px", display: "flex" } },
									sourcesData.registries.map((src) =>
										el("div", { key: src.id, className: styles.rowTop, style: { justifyContent: "space-between" } },
											el("div", { className: styles.rowTop },
												el("strong", { className: styles.name }, src.name + (src.primary ? "（" + t("sourcePrimary") + "）" : "")),
												el("code", null, src.url)),
											el("div", { className: styles.rowTop },
												...
												el("button", { type: "button", className: styles.trashBtn, title: t("removeSource"), "aria-label": t("removeSource"), disabled: sourcesBusy, onClick: () => sourcesAction({ action: "remove", id: src.id }) }, "✕"))))),
							el("div", { className: styles.rowTop },

	...
	factory: (require) => {
		...
		function PluginConsoleTab({ t }) {
			...
			const search = () => {
				...
				if (searchSource === "github" && !multiSource && indexData !== null && indexData.loaded && query.trim() !== "") {
				...
				toolbarOpen
					? el("div", { className: styles.toolbarGroup },
						...
						el("button", {
							...
							onClick: () => { setRepoLandOn((v) => { const n = !v; try { localStorage.setItem("pc-repo-land", n ? "1" : "0"); } catch {} return n; }); setRepoLandOpen(true); },
						}, t("repoLandLabel")))
					: null,

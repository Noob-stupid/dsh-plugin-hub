window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			aiConsentRemember: "以后不再提醒（自动同意 AI 兜底，可能产生费用）",
		...
		const en = {
			...
			aiConsentRemember: "Don't ask again (auto-approve the AI fallback, may incur costs)",
					...
					if (job.stage === "ai-consent") {
						...
						if (aiRemember) {
							...
							return el("div", { key: job.jobId, className: styles.detail },
								el("div", { className: styles.rowTop },
									el("span", { className: styles.spinner }),
									el("strong", { className: styles.name }, t("installingLocal") + "：" + (job.packageName ?? job.repo))),
								el("p", { className: styles.message }, t("aiRememberNote")));
						}
						...
						return el("div", { key: job.jobId, className: styles.detail },
							...
							el("label", { className: styles.consentRemember },
								...
								t("aiConsentRemember")),
							el("div", { className: styles.rowTop },
								el("button", { type: "button", className: styles.toggle, onClick: () => { try { localStorage.setItem("pc-ai-remember", aiRemember ? "1" : "0"); } catch {} aiConsent(job.jobId, true); } }, t("aiConsentApprove")),

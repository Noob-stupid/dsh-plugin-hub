window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 只取仓库的 npm 包名（跳过 README 与 api.github.com，供"添加到本地"快捷路径使用）。 */
		...
		function PluginConsoleTab({ t }) {
			...
			const install = (repo, packageName) => {
				...
				call("/plugin-console/install", { repo, packageName }).then(
					(data) => {
						setMessage(t("installed") + "：" + data.packageName + "（" + data.entryId + "）。" + t("installNote")); setReloadHint(true);
						setTimeout(refresh, 2000);
					},
					(error) => setMessage(t("failed") + "：" + friendlyGithubError(error).message),
				).finally(() => setInstalling(null));
			};
			/** 搜索结果卡片上的"添加到本地"：浏览器直连取包名 → 宿主安装，失败回退宿主取包名。 */
			const addLocal = (item) => {
				if (installing !== null) return;
				setInstalling(item.fullName);
				setMessage(null);
				const installWith = (packageName) => {
					if (packageName === null || packageName === undefined) {
						setMessage(t("failed") + "：" + t("needPackage"));
						setInstalling(null);
						return;
					}
					call("/plugin-console/install", { repo: item.fullName, packageName }).then(
						(data) => {
							setMessage(t("installed") + "：" + data.packageName + "（" + data.entryId + "）。" + t("installNote")); setReloadHint(true);
							setInstalling(null);
							setTimeout(refresh, 2000);
				...
				fetchPackageName(item.fullName, item.defaultBranch).then(
					...
					() => call("/plugin-console/repo", { repo: item.fullName }).then(
						(data) => installWith(data.packageName),
						(error) => {
							setMessage(t("failed") + "：" + friendlyGithubError(error).message);
							setInstalling(null);
						},
			...
			return el("section", { className: styles.section, "aria-busy": state.status === "loading" },
				...
				showBackTop && market !== null && market.status === "ready" && market.data.length > 0
					? el("button", {
						type: "button",
				...
				market !== null && market.status === "ready" && market.data.length >= 20
					? el("div", { className: styles.descTopbar },
						el("button", {
							type: "button",
							className: styles.toggle,

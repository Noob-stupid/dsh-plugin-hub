window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PluginConsoleTab({ t }) {
			...
			// 静态插件索引（dsh-plugin topic 全量浏览）：null=未加载 / {items, loaded}
			...
			react.useEffect(() => {
				call("/plugin-console/market-index").then(
					(data) => { if (data && Array.isArray(data.items)) setIndexData({ items: data.items, loaded: true }); },
					() => {},
				);
			}, []);
			...
			const loadMore = () => {
				...
				if (market.fromIndex === true && indexData !== null) {
					// 索引浏览：本地继续展示更多
					const more = indexData.items.slice(0, indexShown + next * 50);

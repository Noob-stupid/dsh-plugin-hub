window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			pendingCompatHint: "框架升级后被强制禁用：需先点击「更新并适配」，更新完成并通过框架兼容校验后自动解锁",
			updateAndAdapt: "更新并适配",
			pendingCompatBanner: "框架升级后有 {count} 个插件待适配（{from} → {fw}）已强制禁用：点「更新并适配」，更新完成自动校验解锁",
			...
			repoLandLabel: "仓库落地",
			repoLandTitle: "仓库落地：从 GitHub 克隆任意项目到本地（~/.dsh/repos/，不限于 DSH 插件）",
			repoLandPlaceholder: "输入 owner/repo 或 GitHub 链接，如 deepseek-ai/deepseek-harness",
		...
		const en = {
			...
			pendingCompatHint: "Force-disabled after framework upgrade: click Update & adapt, then it unlocks after passing the framework compat check",
			updateAndAdapt: "Update & adapt",
			pendingCompatBanner: "{count} plugins pending adapt after framework upgrade ({from} -> {fw}), force-disabled: click Update & adapt to unlock",
			...
			repoLandTitle: "Repo land: clone any GitHub project to ~/.dsh/repos/ (not limited to DSH plugins)",
			repoLandPlaceholder: "owner/repo or GitHub URL, e.g. deepseek-ai/deepseek-harness",
				...
				? state.data.entries.filter((entry) => {
					...
					return el("li", { key: entry.entryId, className: styles.row },
						el("div", { className: styles.rowTop },
							...
							entry.pendingCompat ? el("span", { className: styles.tag, "data-pending": "true", title: t("pendingCompatHint") }, `${t("pendingCompatTag")} ${state.data.compatPending?.frameworkVersion ?? ""}`) : null,
						),
			...
			return el("section", { ref: sectionRef, className: styles.section, "aria-busy": state.status === "loading" },
				...
				repoLandOpen
					? el("div", { className: styles.modalBackdrop },
						el("div", { className: styles.modalCard, style: { maxWidth: 560 } },
							...
							el("p", { className: styles.message }, t("repoLandTitle")),
				...
				fwShowCard
					? el("div", { className: styles.detail },
						...
						(state.status === "ready" && state.data.rollback)
							? el("div", { className: styles.rowTop },
								el("button", {
									type: "button",
									className: styles.toggle,
...
(error) => setMessage(t("failed") + "：" + friendlyGithubError(error).message),
				...
				toolbarOpen
					? el("div", { className: styles.toolbarGroup },
						el("button", {
							type: "button",
				...
				el("div", { id: "pc-installed-search-area" },
					el("div", { className: styles.installedHead },
						...
						mode === "plugins"
							...
							: null),
					(state.status === "ready" && state.data.compatPending && (state.data.compatPending.pending ?? []).length > 0 && mode === "plugins")
						? el("p", { className: styles.status, "data-pending": "true" }, t("pendingCompatBanner")
							.replace("{count}", String(state.data.compatPending.pending.length))
							.replace("{from}", String(state.data.compatPending.upgradeFrom ?? "?"))
							.replace("{fw}", String(state.data.compatPending.frameworkVersion ?? "?")))
						: null,

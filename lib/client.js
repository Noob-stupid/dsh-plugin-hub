window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const en = {
			...
			collapseMarket: "Collapse market",
			expandMarket: "Expand market",
		...
		function PluginConsoleTab({ t }) {
			...
			return el("section", { className: styles.section, "aria-busy": state.status === "loading" },
				...
				market !== null && market.status === "ready" && market.data.length > 0
					? el("div", { className: styles.backTopWrap },
						...
						el("button", {
							type: "button",
							className: styles.backTop,
							title: marketCollapsed ? t("expandMarket") : t("collapseMarket"),
							"aria-label": marketCollapsed ? t("expandMarket") : t("collapseMarket"),
							...
							marketCollapsed
								...
								: el("polyline", { points: "18 15 12 9 6 15" }))))
					: null,
				!marketCollapsed ? el("p", { className: styles.note }, t("direct")) : null,
				!marketCollapsed ? el("div", { className: styles.search, id: "pc-market-search" },
					el("input", {
						type: "search",
						value: query,
				...
				!marketCollapsed && market !== null && market.status === "ready" && market.data.length >= 20
					...
					: null,
				!marketCollapsed ? Object.values(jobs).filter((job) => job.status === "installing").map((job) => {

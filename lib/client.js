			...
			return el("section", { className: styles.section, "aria-busy": state.status === "loading" },
				...
				state.status === "ready"
					? el("div", { className: styles.detail },
						...
						comps.length === 0
							? el("p", { className: styles.status }, t("noComponents"))
							: el("div", { style: { flexDirection: "column", gap: "6px", display: "flex" } },
								comps.map((c) =>
									el("div", { key: c.id, className: styles.rowTop },
										el("strong", { className: styles.name, style: { flex: "none" } }, c.name),

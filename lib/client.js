window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			githubLoggedOut: "未登录 GitHub（匿名搜索 10 次/分钟；用 dsh-github-login 登录后提升配额）",
		};
		const en = {
			tab: "Plugin console",
			...
			githubLoggedOut: "Not signed in to GitHub (anonymous search, 10 req/min; use dsh-github-login to raise the quota)",
		};
		//#endregion
		...
		function PluginConsoleTab({ t }) {
			...
			return el("section", { className: styles.section, "aria-busy": state.status === "loading" },
				...
				el("p", { className: styles.note },
					state.status === "ready" && state.data.github && state.data.github.loggedIn
						? t("githubLoggedIn") + (state.data.github.login ?? "unknown") + "（服务端回退通道带认证）"
						: t("githubLoggedOut")),

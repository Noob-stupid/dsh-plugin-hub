					...
					call("/plugin-console/check-update", { packageName: "@deepseek-ai/dsh" }).then(
						(data) => {
							if (data && data.error === null) {
								...
								// 版本号比较（防 current 已是 rc.8 时误判 rc.7 为升级目标；rc.N 数字比较）
								const verNum = (v) => { const m = String(v).match(/0\.1\.0-rc\.(\d+)/u); return m ? parseInt(m[1], 10) : -1; };
								// 稳定版 latest 优先；latest 不高于当前而 next（预发布渠道）更高时，目标取 next
								const target = data.latest && verNum(data.latest) > verNum(current)
									? data.latest
									: (data.next && verNum(data.next) > verNum(current) ? data.next : null);

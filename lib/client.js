window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			familyUnlockAllTip: "对全家桶内所有源码扫描通过（已适配）的子包批量解锁启用；未通过的保持禁用",
			familyUnlockAllConfirm: "将对全家桶内所有已适配（源码扫描通过）的子包一键解锁启用？未通过的将保持禁用。",
		...
		const en = {
			...
			familyUnlockAllTip: "Batch-unlock every sub package in this family whose source scan passed; failed ones stay disabled",
			familyUnlockAllConfirm: "Unlock all adapted (source-scan passed) sub packages in this family? Failed ones stay disabled.",
		...
		function PluginConsoleTab({ t }) {
				...
				call("/plugin-console/market-index").then(
					(data) => {
						if (data && Array.isArray(data.items)) {
							setIndexData({ items: data.items, skills: Array.isArray(data.skills) ? data.skills : [], loaded: true });
							// ★「只看官方」需要官方标记：索引条目补标（网络/服务端此前判定失败时不动已有标记，避免 null 覆盖）
							// 服务端 /enrich 已带 24h 结果缓存时，此处仅兜底浏览器直连失败部分。
							const base = data.items;
							enrichOfficialBundle(base).then((enriched) => {
								if (!Array.isArray(enriched)) return;
								const merged = enriched.map((e, i) => {
									const b = base[i] ?? {};
									return {
										...e,
								...
								setIndexData((prev) => (prev !== null && prev.loaded === true ? { ...prev, items: merged } : prev));
							});
						}
					},
					() => {},
						...
						if (details.status === "loading") {
							...
							detailPanel = el("div", { className: styles.detail },
								...
								entry.pendingCompat === true
									? el("p", { className: styles.status, "data-pending": "true" }, t("pendingCompatHint") + (compatInfo && compatInfo.checkNote ? "；已知校验：" + compatInfo.checkNote : ""))
									: null,
					...
					return el("li", { key: entry.entryId, className: styles.row },
						el("div", { className: styles.rowTop },
							el("strong", { className: styles.name, title: entry.moduleName }, (() => {
								...
								return famRoot !== null && (familyCounts.get(famRoot) ?? 0) > 1 && mn.startsWith(famRoot + "/")
			...
			for (const entry of filteredEntries) {
				const root = famRootOf(entry);
				if (root === null || (familyCounts.get(root) ?? 0) <= 1 || familySeen.has(root)) continue;
				familySeen.add(root);
				familyHeaders.push(el("div", { key: "family-" + root, className: styles.detail, style: { border: "1px solid var(--dsw-alias-state-business-primary)", borderRadius: 10, padding: "8px 12px", gridColumn: "1 / -1" } },
					el("div", { className: styles.rowTop, style: { alignItems: "center" } },
						...
						el("span", { className: styles.tag, "data-user": "true" }, t("familyTag") + " · " + (familyCounts.get(root) ?? 0) + " " + t("familySubs")),
						el("button", {
							type: "button",
							className: styles.toggle,
						...
						el("button", {
							...
							onClick: () => {
								...
								call("/plugin-console/check-update", { packageName: root }).then(
									(d) => setMessage(t("familyUpdate") + "：" + root + " " + (d?.latest ?? "?") + (d?.migrate && d.migrate.compatible ? " → " + d.migrate.to + " v" + d.migrate.latest : "") + (d?.error ? "（" + d.error + "）" : "")),
						...
						el("button", {
							...
							onClick: () => {
								...
								call("/plugin-console/adapt-unlock-all", { root }).then(
									(d) => {
										setMessage(t("familyUnlockAllDone") + "：" + (d.unlocked ?? []).length + " 行" + (Array.isArray(d.kept) && d.kept.length > 0 ? "；保留 " + d.kept.length + " 行（未通过校验）" : ""));
										window.setTimeout(() => window.location.reload(), 1500);
									},
									(e) => setMessage(t("failed") + "：" + friendlyGithubError(e).message),
								);
			...
			familyCards.forEach((card, i) => {
				...
				if (root === null || (familyCounts.get(root) ?? 0) <= 1) { nonFamilyCards.push(card); return; }
			...
			const rows = state.status === "ready" ? [...nonFamilyCards, ...familySectionEls] : [];

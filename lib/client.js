window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PluginConsoleTab({ t }) {
					...
					call("/plugin-console/check-update", { packageName: "@deepseek-ai/dsh" }).then(
						(data) => {
							if (data && data.latest && data.error === null) {
								...
								if (data.latest !== current) setUpdateMap((prev) => ({ ...prev, [fwItem.fullName]: data.latest }));
							}
						},
						() => {},
					);
			...
			/** 框架升级：Hub 打包备份现有配置 + 版本检测 + 升级指引（框架本体升级由官方方式执行）。 */
					...
					(data) => {
						...
						if (data && data.ok === true) {
							setMessage(t("frameworkUpgradeDone")
								+ (data.hasUpdate === true ? "：" + (data.current ?? "?") + " → " + (data.latest ?? "?") : t("frameworkUpgradeUpToDate"))
							...
							return el("li", { key: item.fullName, className: styles.item },
								...
								el("div", { className: styles.rowTop },
									...
									item.skillTopics !== undefined
										...
										: item.fullName === "deepseek-ai/deepseek-harness" && updateMap[item.fullName] !== undefined

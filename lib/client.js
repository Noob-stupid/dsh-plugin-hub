window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PluginConsoleTab({ t }) {
			...
			// AI 兜底开关（默认开）：关掉后常规通道失败即取消，不调用模型 API（零费用）
			const [aiFallback, setAiFallback] = react.useState(() => {
				try { return localStorage.getItem("pc-ai-fallback") !== "off"; } catch { return true; }
			});
							...
							if (data.status !== "installing") {
								...
								if (data.status === "done") {
									if (data.ai === true) {
										...
										if (data.bundle) setRestartHint(true);
										else setReloadHint(true);
									}
								} else {
				...
				el("div", { className: styles.search, id: "pc-market-search" },
					...
					el("button", {
						...
						onClick: () => setAiFallback((v) => {
							const next = !v;
							try { localStorage.setItem("pc-ai-fallback", next ? "on" : "off"); } catch {}
							return next;
						}),
					}, t("aiFallbackLabel")),
				),
					...
					if (job.stage === "ai-consent") {
						if (!aiFallback) {
							...
							return el("div", { key: job.jobId, className: styles.detail },
								...
								el("p", { className: styles.message }, t("aiFallbackOff")));
						}
						// 本地 AI 兜底会调用模型 API 产生费用：必须先取得用户同意
						return el("div", { key: job.jobId, className: styles.detail },
							el("div", { className: styles.rowTop },
								el("span", { className: styles.spinner }),
								el("strong", { className: styles.name }, t("installingLocal") + "：" + (job.packageName ?? job.repo))),
							...
							el("div", { className: styles.rowTop },
								el("button", { type: "button", className: styles.toggle, onClick: () => aiConsent(job.jobId, true) }, t("aiConsentApprove")),

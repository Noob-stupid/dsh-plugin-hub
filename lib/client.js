				...
				fwShowCard
					? el("div", { className: styles.detail },
						...
						el("ul", { className: styles.market }, FW_STEPS.map((step, idx) => {
							...
							// v0.3.37：失败时按脚本记下的「崩溃前最后阶段」标步骤——已完成的显示 ✓，
							// 真正失败的那一步显示 ✕，后面的显示「未执行」。旧记录没有 stage 才退回整列 ✕；
							// 但若框架本体已在目标版本（frameworkAtTarget），说明安装类步骤确实都过了，
							// 按「停在重启服务那一步」显示，比整列红叉诚实。
							const failedStage = frameworkStatus.status === "failed"
								? (frameworkStatus.stage ?? (frameworkStatus.frameworkAtTarget ? "relaunching" : null))
								: null;
							const stageIdx = failedStage ? FW_STEPS.findIndex((s) => s.key === failedStage) : -1;
							const state2 = frameworkStatus.status === "done"
								? "ok"
								: frameworkStatus.status === "failed"

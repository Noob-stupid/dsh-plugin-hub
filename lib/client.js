window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PluginConsoleTab({ t }) {
			...
			const aiAutoShownRef = react.useRef(false);
				...
				try {
					...
					if (data && data.ok === true) {
						...
						if (data.status === "plan-ready") {
							...
							setAiSelected(sel);
							// 计划就绪 = 同意窗口：自动弹出确认界面（仅一次，用户关掉后不再强弹）
							if (!aiAutoShownRef.current) {
								aiAutoShownRef.current = true;
								setAiOpen(true);
								setToolbarOpen(true);
							}
						}
...
(error) => setMessage(t("failed") + "：" + friendlyGithubError(error).message),
				...
				el("button", {
					...
					aiRunning
						? el("div", { className: styles.aiSpinner, title: t("aiProgressTitle"), "aria-label": t("aiProgressTitle"), onClick: (event) => { event.stopPropagation(); setAiOpen(true); setToolbarOpen(true); } })
						: null,

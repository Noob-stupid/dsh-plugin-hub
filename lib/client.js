window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			officialEmpty: "没有可走官方通道的插件。",
			officialChecking: "正在识别官方通道…",
			elapsed: "已用时",
		};
		const en = {
			tab: "Plugin console",
			...
			stageRepairing: "Local AI repair (automatic fallback)",
			...
			officialEmpty: "No official-channel plugins.",
			officialChecking: "Checking official channel…",
			elapsed: "Elapsed",
		};
		//#endregion
		...
		/** 批量识别搜索结果是否为官方通道插件：读每个仓库根 package.json 的 dsh.bundle 清单。 */
		async function enrichOfficialBundle(items) {
			return Promise.all(items.map(async (item) => {
				...
				if (pkgText !== null) {
					try {
						const pkg = JSON.parse(pkgText);
						official = typeof pkg.dsh?.bundle?.patch === "string";
					} catch {}
				}
		...
		function PluginConsoleTab({ t }) {
												...
												call("/plugin-console/uninstall", { entryId: entry.entryId }).then(
													(data) => {
														...
														if (data && data.restart === true) {
															setMessage(t("deleteBundleNote") + "：" + (data.packageName ?? ""));
															setRestartHint(true);
														} else {
															setMessage(t("deleteNote") + "：" + (data.packageName ?? ""));
															setTimeout(refresh, 1500);
														}
													},
													(error) => setMessage(t("failed") + "：" + friendlyGithubError(error).message),
			...
			const marketShown = market === null || market.status !== "ready"
				? []
				: officialOnly
					? market.data.filter((item) => item.official === true)
					: market.data;

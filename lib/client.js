		...
		function PluginConsoleTab({ t }) {
					...
					call("/plugin-console/check-update", { packageName: "@deepseek-ai/dsh" }).then(
						(data) => {
							if (data && data.error === null) {
								...
								// 稳定版 latest 优先；latest 与当前相同而 next（预发布渠道）更新时，目标取 next
								const target = data.latest && data.latest !== current
									? data.latest
									: (data.next && data.next !== current ? data.next : null);
					...
					(data) => {
						...
						if (data && data.ok === true) {
							setMessage(t("frameworkUpgradeDone")
								+ (data.hasUpdate === true ? "：" + (data.current ?? "?") + " → " + (data.target ?? data.latest ?? "?") : t("frameworkUpgradeUpToDate"))
			...
			return el("section", { className: styles.section, "aria-busy": state.status === "loading" },
				...
				state.status === "ready" && state.data.framework && state.data.framework.upgraded === true && !frameworkNoticeDismissed
					...
					: null,
				frameworkStatus !== null && frameworkStatus.status !== "idle" && frameworkStatus.status !== "done" && !fwStatusDismissed
					? el("button", {
						type: "button",
					...
					: null,
				frameworkStatus !== null && frameworkStatus.status !== "idle" && !fwStatusDismissed
					? el("div", { className: styles.detail },
						el("div", { className: styles.rowTop },
							...
							el("button", { type: "button", className: styles.trashBtn, title: t("closeModal"), "aria-label": t("closeModal"), onClick: () => setFwStatusDismissed(true) }, "✕")),

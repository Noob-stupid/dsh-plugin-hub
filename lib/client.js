window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const en = {
			...
			aiEmpowerRun: "Run selected steps",
		...
		function PluginConsoleTab({ t }) {
			...
			async function aiStartPlan() {
				...
				setAiRunning(true);				try {
					...
					if (data && data.ok === true) { setAiJob({ jobId: data.jobId, source, status: "running", stage: "planning" }); aiPoll(data.jobId); }
			...
			async function aiRun() {
				...
				setAiRunning(true);
				try {
					await call("/plugin-console/ai-empower/run", { jobId: aiJob.jobId, steps });
					aiPoll(aiJob.jobId);
				} catch (error) {
					setAiRunning(false);
					setMessage(t("failed") + "：" + friendlyGithubError(error).message);
			...
			react.useEffect(() => {
				if (state.status === "ready") refreshComps();
			}, [state.status]);
			...
			return el("section", { className: styles.section, "aria-busy": state.status === "loading" },
				...
				state.status === "ready" && comps.length > 0 && details === null && repoInfo === null
					? el("div", { className: styles.compCard },
						el("div", { className: styles.rowTop },
							...
							aiJob === null || (aiJob.status === "running" && aiJob.stage === "planning")
								? el("div", { style: { flexDirection: "column", gap: "10px", display: "flex" } },
									...
									el("div", { className: styles.rowTop },
										...
										aiRunning ? el("span", { className: styles.message }, t("aiEmpowerWaiting")) : null)))
								: aiJob.status === "plan-ready"
									? el("div", { style: { flexDirection: "column", gap: "10px", display: "flex" } },
										...
										el("div", { className: styles.rowTop },
											...
											el("button", { type: "button", className: styles.toggle, onClick: () => { setAiJob(null); setAiRunning(false); } }, t("aiEmpowerCancel")))))
									: el("div", { style: { flexDirection: "column", gap: "10px", display: "flex" } },
										el("p", { className: styles.message, "data-error": aiJob.status === "failed" ? "true" : undefined }, aiJob.status === "done" ? t("aiEmpowerDone") : aiJob.status === "failed" ? t("aiEmpowerFail") + "：" + (aiJob.error ?? "?") : t("aiEmpowerExecuting") + "（" + (aiJob.progress?.done ?? 0) + "/" + (aiJob.progress?.total ?? "?") + "）"),
										...
										el("div", { className: styles.rowTop },
											(aiJob.status === "running") ? el("button", { type: "button", className: styles.toggle, onClick: aiCancel }, t("aiEmpowerCancel")) : null,
											el("button", { type: "button", className: styles.toggle, onClick: () => { setAiJob(null); } }, t("closeModal"))))))
					: null,
